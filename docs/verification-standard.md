...
## 9. 多实例（verify-multi-instance，2026-09 追加）

| 用例 | 断言 | 结果 |
|------|------|------|
| MI-01/05 | 双实例并行；配置端口被占 → 自动端口回退（日志实据） | ✔ |
| MI-02 | instances/<id>/config 与证书目录各自独立 | ✔ |
| MI-03 | 顶栏配色 = 实例色，两实例互不相同 | ✔ |
| MI-04 | 同一 relay 双通道双控制连接在线，无接管 | ✔ |

运行：`npm run verify:instances`（真实 Electron 双开，约 40s）
