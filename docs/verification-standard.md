...
| `scripts/verify-browser-e2e.mjs` | **真浏览器全流程**（`npm run verify:browser`）：未认证重定向、登录表单与指纹核对、错/对密码、标签栏与 UI 按钮、cookie 三属性、xterm 键入→PTY→bash→渲染回显、**刷新后恢复且不新建标签**；产截图 `docs/test-reports/screenshots/` | 同上 |
